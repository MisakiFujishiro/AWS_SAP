...
### ネットワークのパフォーマンス
ネットワーク帯域(クライアントからEC2)については、インスタンスタイプによって定まる。ネットワーク帯域がボトルネックになっている場合は、インスタンスタイプを大きくする必要がある。
...
### プレスメントグループ
インスタンスの配置戦略により、配置を分散させることで障害発生時の影響を抑えることができる。一方で配置を集中させることでネットワークレイテンシ−を低減することができる。プレイスメントグループへEC2を所属させる場合は、EC2が停止している状態から起動するときに設定する
...
### EBS
####  I/Oクレジット
EBSにもI/Oクレジットがあり、これを超えるとIOPSの性能が落ちる。
I/OクレジットはEBSのサイズやタイプに依存している。

[AWS の IOPS と I/O クレジットがよくわからなかったので整理した](https://stakiran.hatenablog.com/entry/2018/06/25/190920#IO-%E3%82%AF%E3%83%AC%E3%82%B8%E3%83%83%E3%83%88%E3%81%9D%E3%81%AE1-%E6%B6%88%E8%B2%BB%E3%81%A8%E8%A3%9C%E5%85%85%E3%81%AE%E3%83%AB%E3%83%BC%E3%83%AB)

#### 最適化オプション
EC2とEBSのネットワークをパフォーマンスを高めることができる
...
#### AutoScaling
DyanamoDBにはオートスケーリング機能があり、スループットを変更することが可能。
プロビジョンドとAutoScalingを組み合わせるとトラフィックの変動に応じて、プロビジョンドスループット設定を自動的に調整することでスロットリングに対応できる。
- [DynamoDBにAutoScalingを適用した時の挙動](https://qiita.com/Nazuna_Nyanmage/items/443ee3d196d6ae600c9a)
#### DAX
DynamoDBのキャッシングが可能
...
また、マルチAZ構成が可能で可能で高可用性を担保することができる。
...
#### セキュリティ
デフォルトで、AWS Shieldが有効になっているので、DDos攻撃を緩和可能。
